...
class CVP_LIVE_FILTER_SORTBY extends CVP_LIVE_FILTER {
	...
	function show_as_filter( $args ) {
		...
		if ( is_array( $this->selected_filters ) ) {
			foreach ( $this->selected_filters as $idx => $field ) {
				...
				$field_vals[ $field . $suffix ] = __cvptranslate( $this->get_label( 'live-filter-heading', $idx, $field ), "sort - $field - heading" );
			}
		}
		...
		if ( $field_vals ) {
			$type	 = 'dropdown';
			$others	 = array(
				'label'			 => __cvptranslate( PT_CV_Functions::setting_value( PT_CV_PREFIX . 'livesort-live-filter-heading' ), "sort - heading" ),
				'show_count'	 => false,
				'hide_empty'	 => false,
				'default_text'	 => __cvptranslate( PT_CV_Functions::setting_value( PT_CV_PREFIX . 'livesort-default-text' ), "sort - placeholder" ),
			);
	...
	function get_common_sortby() {
		...
		if ( $selected_options ) {
			...
			$texts	 = __cvptranslate( PT_CV_Functions::setting_value( PT_CV_PREFIX . 'livesort-options-text' ), "sort - texts" );
