...
class CVP_LIVE_FILTER_SEARCH extends CVP_LIVE_FILTER {
    ...
    function show_as_filter( $args ) {
        if ( $this->selected_filters ) {
            $type   = 'search_field';
            $others = array(
                'label'       => __cvptranslate( PT_CV_Functions::setting_value( PT_CV_PREFIX . 'search-live-filter-heading' ), "search - heading" ),
                'value'       => '',
                'placeholder' => __cvptranslate( PT_CV_Functions::setting_value( PT_CV_PREFIX . 'search-live-filter-placeholder' ), "search - placeholder" ),
            );
